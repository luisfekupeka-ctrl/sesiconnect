    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {atividadesDoDia.map((ativ) => (
          <AfterCard key={ativ.id} ativ={ativ} expandida={atividadeAberta === ativ.id} onToggle={() => setAtividadeAberta(atividadeAberta === ativ.id ? null : ativ.id)} />
        ))}
...
function AfterCard({ ativ, expandida, onToggle }: { ativ: any, expandida: boolean, onToggle: () => void }) {
  const [busca, setBusca] = useState('');
  const alunosNoBloco = ativ.listaAlunos || [];
  const alunosFiltrados = alunosNoBloco.filter((a: string) => a.toLowerCase().includes(busca.toLowerCase()));
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        expandida ? "border-amber-500 bg-amber-500/5 shadow-[0_0_50px_rgba(245,158,11,0.2)]" : "border-[#30363d] hover:border-amber-500/30 shadow-xl"
      )}
    >
            ...
            {expandida && (
              ...
              >
                  ...
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                      alert('Abrindo chamada digital para ' + ativ.nome);
                    }}
