...
export default function Dashboard() {
  ...
  return (
    ...
    >
      <header>
        <h1 className="text-3xl md:text-4xl font-black tracking-tight text-accent-cyan">Agora</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5">
        {[
          { rotulo: 'Aulas', valor: totalOcupadas, icone: Clock, cor: 'text-accent-cyan', bg: 'bg-cyan-900/20' },
          { rotulo: 'After', valor: afterAtivo, icone: Sparkles, cor: 'text-accent-amber', bg: 'bg-amber-900/20' },
          { rotulo: 'Alunos', valor: totalAlunos, icone: UserCheck, cor: 'text-accent-emerald', bg: 'bg-emerald-900/20' },
          { rotulo: 'Ocorrências', valor: ocorrenciasHoje, icone: AlertTriangle, cor: 'text-accent-rose', bg: 'bg-rose-900/20' },
        ].map((m, i) => (
          <div key={i} className="bg-surface p-5 md:p-7 rounded-2xl border border-cyan-900 flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl md:text-4xl font-black text-accent-cyan">{m.valor}</p>
            </div>
      ...
      <div className="bg-surface border-2 border-dashed border-cyan-800 rounded-2xl p-5 md:p-7 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 bg-cyan-900/30 text-accent-cyan rounded-2xl flex items-center justify-center border border-cyan-800">
            <Activity size={28} />
          </div>
          <div>
            <p className="text-[11px] font-bold text-on-surface-variant uppercase tracking-wider mb-1">Próxima Transição</p>
            <h3 className="text-xl md:text-2xl font-black tracking-tight text-accent-cyan">{estadoEscola.rotuloProximaTransicao}</h3>
          </div>
        </div>
        <div className="flex flex-col items-center md:items-end text-center md:text-right">
          <p className="text-[11px] font-bold text-on-surface-variant uppercase tracking-wider mb-1">Horário</p>
          <p className="text-3xl md:text-5xl font-black text-accent-cyan tracking-tight">{estadoEscola.proximaTransicao}</p>
        </div>
      </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between px-1">
          <h2 className="text-xl md:text-2xl font-black tracking-tight text-accent-cyan">
            Salas Ocupadas
          </h2>
          {totalPaginas > 1 && (
            ...
            {salasExibidas.map((sala) => (
              <div key={sala.numeroSala} className="bg-surface p-6 md:p-7 rounded-2xl border border-cyan-900 group hover:border-accent-cyan transition-all">
                <div className="flex items-center justify-between mb-6">
                   <div className="w-12 h-12 bg-cyan-900/30 rounded-2xl flex items-center justify-center text-accent-cyan group-hover:bg-cyan-900/50 transition-all">
                     <DoorOpen size={24} />
                   </div>
                   <div className="text-right">
                     <p className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider">Sala</p>
                     <p className="text-xl font-black text-accent-cyan">{sala.numeroSala.toString().padStart(2, '0')}</p>
                   </div>
                </div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <h3 className="text-base font-bold leading-tight text-on-surface">{sala.materiaAtual}</h3>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <div className="pt-3 border-t border-cyan-900 flex justify-between items-center">
                    ...
                    <span className="text-xs font-bold text-accent-cyan">{sala.horarioFim}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          {estadoEscola.salas.map((sala) => (
            <div
              ...
              className={cn(
                ...
                sala.estaOcupada 
                  ? "bg-accent-cyan text-white border border-cyan-400 scale-105" 
