  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 px-4">
        {languageLab.map((lab) => (
          <LabCard key={lab.id} lab={lab} selecionado={labSelecionado === lab.id} onSelect={() => setLabSelecionado(labSelecionado === lab.id ? null : lab.id)} />
        ))}
      </div>
    </motion.div>
  );
...
function LabCard({ lab, selecionado, onSelect }: { lab: any, selecionado: boolean, onSelect: () => void }) {
